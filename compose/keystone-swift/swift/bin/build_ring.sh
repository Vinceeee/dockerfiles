...
for dev in $DEVICES;do
    for host in $HOSTS;do
        swift-ring-builder account.builder add --ip $host --region 1 --zone $i --port 6012 --device $dev 100
        swift-ring-builder container.builder add --ip $host --region 1 --zone $i --port 6011 --device $dev 100
        swift-ring-builder object.builder add --ip $host --region 1 --zone $i --port 6010 --device $dev 100
        swift-ring-builder object-1.builder add --ip $host --region 1 --zone $i --port 6010 --device $dev 100
    done
    let i+=1
done
IFS=$OLD_IFS
